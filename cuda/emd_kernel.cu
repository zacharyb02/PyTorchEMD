...
at::Tensor ApproxMatchForward(
  ...
  TORCH_CHECK(xyz2.size(0) == b, "Mismatch: xyz2.size(0) != b");
  TORCH_CHECK(xyz1.size(2) == 3, "Mismatch: xyz1.size(2) != 3");
  TORCH_CHECK(xyz2.size(2) == 3, "Mismatch: xyz2.size(2) != 3");
  CHECK_INPUT(xyz1);
  CHECK_INPUT(xyz2);
...
at::Tensor MatchCostForward(
  ...
  TORCH_CHECK(xyz2.size(0) == b, "xyz2.size(0) must be equal to b");
  TORCH_CHECK(xyz1.size(2) == 3, "xyz1 must have 3D coordinates (size(2) == 3)");
  TORCH_CHECK(xyz2.size(2) == 3, "xyz2 must have 3D coordinates (size(2) == 3)");
  CHECK_INPUT(xyz1);
  CHECK_INPUT(xyz2);
...
std::vector<at::Tensor> MatchCostBackward(
  ...
  TORCH_CHECK(xyz2.size(0) == b, "xyz2.size(0) must be equal to b (batch size mismatch)");
  TORCH_CHECK(xyz1.size(2) == 3, "xyz1 must have 3D coordinates (size(2) == 3)");
  TORCH_CHECK(xyz2.size(2) == 3, "xyz2 must have 3D coordinates (size(2) == 3)");
  CHECK_INPUT(xyz1);
  CHECK_INPUT(xyz2);
